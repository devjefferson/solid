...
import require$$1, { dirname, resolve, join } from 'path';
import _debug from 'debug';
import { green, bold } from 'kolorist';
import sirv from 'sirv';
import { createFilter } from '@rollup/pluginutils';
import require$$0$2, { mkdirSync, readFileSync as readFileSync$1, writeFileSync as writeFileSync$1 } from 'fs';
import require$$0 from 'constants';
import require$$0$1 from 'stream';
import require$$4 from 'util';
import require$$5 from 'assert';
import visualizer from '@vinxi/rollup-plugin-visualizer';
...
`);
  ...
  async function generateBuild() {
    let folder = outDir ? resolve(config.root, outDir) : join(process.cwd(), "node_modules", ".vite", "inspect");
    mkdirSync(folder, { recursive: true });
    lib.copySync(join(_dirname, clientDir), join(folder, "__inspect"));
    let html = readFileSync$1(join(folder, "__inspect", "index.html"), "utf8").replace("DEV", "BUILD");
    writeFileSync$1(join(folder, "__inspect", "index.html"), html);
    writeFileSync$1(join(folder, "__inspect", "route.html"), html);
    writeFileSync$1(join(folder, "__inspect", "asset.html"), html);
    writeFileSync$1(join(folder, "__inspect", "module.html"), html);
    writeFileSync$1(join(folder, "index.html"), html);
    ...
    writeFileSync$1(join(transforms, "_modules.json"), JSON.stringify({
      root,
    ...
    writeFileSync$1(join(transforms, "_metrics.json"), JSON.stringify(getPluginMetrics(Boolean(config.build.ssr)), null, 2));
    modules.forEach((mod, index) => {
      const file = join(transforms, `${index}.json`);
      writeFileSync$1(file, JSON.stringify(mod, null, 2));
    });
  }
  return {
    apply: (config2, mode) => {
    ...
    async generateBundle(...args) {
      let folder = outDir ? resolve(config.root, outDir) : join(process.cwd(), "node_modules", ".vite", "inspect");
      mkdirSync(folder, { recursive: true });
      let fn = typeof visualizer === "function" ? visualizer : visualizer.default;
      return await fn({
        json: true,
        gzipSize: true,
        filename: join(folder, `stats.${config.build.ssr ? "ssr" : "client"}.json`)
