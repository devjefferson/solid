...
const visualizer = require('@vinxi/rollup-plugin-visualizer');
...
const visualizer__default = /*#__PURE__*/_interopDefaultLegacy(visualizer);
...
function retry () {
  ...
  if (elem) {
    debug$1('RETRY', elem[0].name, elem[1]);
...
const _dirname = typeof __dirname !== "undefined" ? __dirname : require$$1.dirname(url.fileURLToPath((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('index.cjs', document.baseURI).href))));
...
function PluginInspect(options = {}) {
  ...
  function resolveId(id = "", ssr) {
    if (id.startsWith("./"))
      id = require$$1.resolve(config.root, id).replace(/\\/g, "/");
    return resolveIdRec(id, ssr);
  }
  function resolveIdRec(id, ssr) {
    const map = ssr ? ssrIdMap : idMap;
    return map[id] ? resolveIdRec(map[id], ssr) : id;
  }
  function getIdInfo(id, ssr = false) {
  ...
  function getPluginMetrics(ssr) {
    const map = {};
    let anonymousPlugin = 0;
    config.plugins.forEach((i) => {
      let name = i.name ?? `(anonymous-${anonymousPlugin++})`;
      map[name] = {
        name,
        enforce: i.enforce,
        invokeCount: 0,
  ...
  function configureServer(server) {
    ...
    server.middlewares.use("/__inspect", sirv__default(require$$1.resolve(_dirname, "../dist/client"), {
      single: true,
      dev: true
    }));
    async function preloadSSRModule(id) {
      try {
    ...
    function list() {
      ...
      return {
        root: config.root,
        modules: Object.keys(modules).sort().map((id) => modules[id])
      };
    }
    function clear(_id) {
...
`);
  ...
  async function generateBuild() {
    let folder = outDir ? require$$1.resolve(config.root, outDir) : require$$1.join(process.cwd(), "node_modules", ".vite", "inspect");
    require$$0$2.mkdirSync(folder, { recursive: true });
    lib.copySync(require$$1.join(_dirname, clientDir), require$$1.join(folder, "__inspect"));
    let html = require$$0$2.readFileSync(require$$1.join(folder, "__inspect", "index.html"), "utf8").replace("DEV", "BUILD");
    require$$0$2.writeFileSync(require$$1.join(folder, "__inspect", "index.html"), html);
    require$$0$2.writeFileSync(require$$1.join(folder, "__inspect", "route.html"), html);
    require$$0$2.writeFileSync(require$$1.join(folder, "__inspect", "asset.html"), html);
    require$$0$2.writeFileSync(require$$1.join(folder, "__inspect", "module.html"), html);
    require$$0$2.writeFileSync(require$$1.join(folder, "index.html"), html);
    const transforms = require$$1.join(folder, "transforms", config.build.ssr ? "ssr" : "client");
    await lib.rm(transforms, {
      recursive: true,
      force: true
    });
    require$$0$2.mkdirSync(transforms, { recursive: true });
    function list() {
      let map = config.build.ssr ? ssrTransformMap : transformMap;
      return {
        root: config.root,
        modules: Object.keys(map).sort().map((id) => ({
          id,
          deps: [],
    ...
    require$$0$2.writeFileSync(require$$1.join(transforms, "_modules.json"), JSON.stringify({
      root,
    ...
    require$$0$2.writeFileSync(require$$1.join(transforms, "_metrics.json"), JSON.stringify(getPluginMetrics(Boolean(config.build.ssr)), null, 2));
    modules.forEach((mod, index) => {
      const file = require$$1.join(transforms, `${index}.json`);
      require$$0$2.writeFileSync(file, JSON.stringify(mod, null, 2));
    });
  }
  return {
    apply: (config2, mode) => {
    ...
    async generateBundle(...args) {
      let folder = outDir ? require$$1.resolve(config.root, outDir) : require$$1.join(process.cwd(), "node_modules", ".vite", "inspect");
      require$$0$2.mkdirSync(folder, { recursive: true });
      let fn = typeof visualizer__default === "function" ? visualizer__default : visualizer__default.default;
      return await fn({
        json: true,
        gzipSize: true,
        filename: require$$1.join(folder, `stats.${config.build.ssr ? "ssr" : "client"}.json`)
      }).generateBundle?.call(this, args[0], args[1], args[2]);
    }
  };
}
